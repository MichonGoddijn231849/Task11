...
class Simulation:
    def __init__(self, num_agents, render=True, rgb_array=False):
        ...
        texture_list = [f for f in os.listdir(texture_folder) if f.endswith(('.png', '.jpg', '.jpeg'))]
        if not texture_list:
            raise FileNotFoundError("No valid texture files found in the 'textures' directory.")
        random_texture = random.choice(texture_list)
        random_texture_index = texture_list.index(random_texture)
        plate_folder = os.path.join(os.path.dirname(__file__), "textures", "_plates")
        self.plate_image_path = os.path.join(plate_folder, os.listdir(plate_folder)[random_texture_index])
        self.textureId = p.loadTexture(os.path.join(texture_folder, random_texture))
        ...
        for i in range(grid_size):
            for j in range(grid_size):
                if agent_count < num_agents:  # Check if more agents need to be placed
                    ...
                    urdf_path = os.path.join(os.path.dirname(__file__), "ot_2_simulation_v6.urdf")
                    robotId = p.loadURDF(urdf_path, position, [0, 0, 0, 1], flags=p.URDF_USE_INERTIA_FROM_FILE)
                    ...
                    urdf_path = os.path.join(os.path.dirname(__file__), "custom.urdf")
                    planeId = p.loadURDF(urdf_path, position_with_offset, rotate_90)
    ...
    def reset(self, num_agents=1):
        ...
        # Remove previous robots, spheres, and specimens
        ...
        # Reinitialize the robots
        self.create_robots(num_agents)
        print("Robot IDs:", self.robotIds)
        print("Pipette Positions:", self.pipette_positions)
